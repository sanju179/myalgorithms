...
class PQueue:
    ...
    def dequeue(self):
        ...
        if self._deqcount % 2 == 0:
            #pop from 0
            if self.is_empty(0):
                raise Empty('High Priority Queue is empty')
            else:
                el = self._high[self._fronth]
                self._high = self._high[1:] + [None]
                self._fronth = (self._fronth + 1) % len(self._high)
                self._highsize -= 1
                self._deqcount += 1
                
                return f"Popped from HP Queue: {el}"
        else:
            #pop from 1
            if self.is_empty(1):
                raise Empty('Low priority Queue is empty')
            else:
                el = self._low[self._frontl]
                self._low = self._low[1:] + [None]
                self._frontl = (self._frontl + 1) % len(self._low)
                self._lowsize -= 1
                self._deqcount += 1
                
                return f"Popped from LP Queue: {el}"
